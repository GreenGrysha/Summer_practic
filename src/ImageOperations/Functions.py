...
def int_check():
    """Запрашивает у пользователя целое число с проверкой ввода."""
    num = input()
    while True:
        try:
            num = int(num)
...
def crop(img: np.ndarray, qt):
    ...
    Args:
        ...
        qt (QtInterface): Интерфейс для взаимодействия с GUI.
    """
...
def rotation(img: np.ndarray, qt):
    ...
    rotate_image = cv2.warpAffine(img_for_redact,
...
def rectangle(img: np.ndarray, qt):
    ...
    rectangle_img = cv2.rectangle(img_for_redact, (x1, y1), (x2, y2),
                                  (255, 0, 0), 3)
    qt.show_image("Прямоугольник", rectangle_img)


def red_channel(img: np.ndarray, qt):
    """Выделяет красный канал изображения.
...
def green_channel(img: np.ndarray, qt):
    """Выделяет зелёный канал изображения.
    ...
    qt.show_image("Зелёный канал", green)


def blue_channel(img: np.ndarray, qt):
    """Выделяет синий канал изображения.
